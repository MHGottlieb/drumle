...
taler$year[grep(" EF ", taler$text, ignore.case=TRUE)]
...
# Men sætningerne bliver kortere ------------------------------------------

plot(taler$words/str_count(taler$text, "\\.")) #gns ord per sætning
...
tomatch <- c(lande, "Grønland", "Færøerne")
tomatch <- c("Klima", "Energi", "Økonomi", "Krig", "Terror", "Miljø", "Europa", " EF | EU ")

result <- lapply(tolower(taler$text), str_count, tolower(tomatch))
